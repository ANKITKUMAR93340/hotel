...
const initialState: State = [
  {
    id: v4(),
    roomName: "Ankit House",
    description: "Feel the Royalty",
    maxGuests: 10,
    coupleBeds: 2,
    singleBeds: 6,
    available: true,
...
export const roomsSlice = createSlice({
  ...
  reducers: {
    ...
    updateRoom: (state, action: PayloadAction<{ updatedRoom: Room }>) => {
      ...
      const newState = [
        ...state.filter((room) => room.id !== updatedRoom.id),
        updatedRoom,
      ];
      return newState;
    },
    ...
    addGuestRoom: (
      state,
      action: PayloadAction<{ newGuest: Guest; roomId: string }>
    ) => {
...
export const { addRoom, removeRoom, updateRoom, addGuestRoom } =
  roomsSlice.actions;
